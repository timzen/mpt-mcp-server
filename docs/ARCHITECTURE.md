...
│  2. POST /api/agents/claim/:id → take ownership                │
│  3. POST /api/agents/transition/:id → advance state            │
│  4. Build prompt from task + instructions + comments           │
│  5. Write MCP config (if adapter.supportsMcp)                  │
│  6. Call adapter.execute(prompt, config) → harness CLI         │
│  7. Check for NEEDS_INPUT: → post comment + release            │
│  8. POST /api/agents/transition/:id → advance (or release)     │
│  9. If more teammate phases → loop back to step 4              │
│                                                                │
...
- **Daemon API contract**: The runner uses the my-pizza-team daemon's agent protocol: `GET /api/agents/next-work`, `POST /api/agents/claim/:id`, `POST /api/agents/transition/:id`, `POST /api/agents/release/:id`, `POST /api/agents/heartbeat`. This multi-transition model supports arbitrary workflow configurations.
...
### transition_task
- **Required**: `taskId` (string), `targetState` (string)
- **Optional**: `result` (string)
- Delegates to: `POST /api/agents/transition/:taskId`
